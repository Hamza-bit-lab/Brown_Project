...
@section('container')
    ...
    <div class="order_operation">
        ...
        Change Order Status
        <select class="form-control m-b-10" id="order_status"
        ...
        <input type="hidden" id="original_payment_status" value="{{ $orders_details[0]->payment_status }}">
    ...
    <div class="row m-t-30 whitebg">
        <div class="col-md-6">
            <div class="order_detail">
                ...
                {{$orders_details[0]->name}}({{$orders_details[0]->mobile}}) <br/>{{$orders_details[0]->address}}
                <br/>{{$orders_details[0]->city}}</br>{{$orders_details[0]->state}}</br/>{{$orders_details[0]->pincode}}
            </div>
        </div>
        <div class="col-md-6">
            <div class="order_detail">
                    ...
                    <div class="table-responsive">
                        <table class="table order_detail">
                            ...
                            @foreach($orders_details as $list)
                                ...
                                <tr>
                                    <td>{{$list->pname}}</td>
                                    <td><img src='{{asset('images/'.$list->attr_image)}}' width="100px" height="150px"/>
                                    </td>
                                    <td>{{$list->size}}</td>
                                    <td>{{$list->color}}</td>
                                    <td>{{$list->price}}</td>
